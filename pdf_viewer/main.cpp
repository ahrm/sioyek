...
float FREETEXT_BOOKMARK_FONT_SIZE = 5.0f;
...
MainWidget* handle_args(const QStringList& arguments) {
	...
	if (should_create_new_window) {
		...
		target_window->command_manager->create_macro_command(target_window, "", STARTUP_COMMANDS)->run();
...
//    format.setSwapInterval(0);
	...
	main_widget->command_manager->create_macro_command(main_widget, "", STARTUP_COMMANDS)->run();
